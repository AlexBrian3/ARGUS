...
-------------------------------------------------------------------------------
Every strong hackathon or builder opportunity is also a great piece of content!

This engine creates ready-to-publish scripts:
1. X / Twitter 5-Post Thread (Hook, Data, Playbook, Build, CTA)
2. TikTok / Instagram Reels Script (45-second high tempo)
3. YouTube Shorts Script
4. LinkedIn Technical Thought-Leadership Post

Calculates CONTENT POTENTIAL SCORE (out of 100).
Strictly in recommendation mode — never posts automatically!
"""
...
class SocialContentEngine:
    """Evaluates discoveries for content virality and drafts multi-platform copy."""
    ...
    def calculate_content_potential(
        ...
        """
        Step 1: Calculate Content Potential Score out of 100 based on 7 factors.
        """
        weights = {
            "novelty": 0.15,
            "timeliness": 0.20,
    ...
    def generate_content_package(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        """
        Step 2: Generate multi-platform drafts for a strong opportunity.
        """
        ...
        # ------------------------------------------------------------------- #
        # Format 1: X / Twitter 5-Post Thread                                 #
        # ------------------------------------------------------------------- #
        lead_sponsor = sponsors[0] if sponsors else "top sponsors"

        x_thread = [
            # Tweet 1: The Contrarian Hook
            f"1/ Most developers entering {name} are going to build the exact same 3 things:\n\n"
            f"- A generic AI chatbot\n- A basic trading bot\n- A toy NFT minter\n\n"
            f"Here is how you actually win the ${total_prize:,.0f} prize pool (backed by real sponsor data): 🧵👇",

            # Tweet 2: The Math Nobody Looks At
            ...
            f"That puts the expected value per serious builder at over ${prize_ratio:,.0f}.\n\n"
            f"This is massively mispriced.",

            # Tweet 3: What Sponsors Repeatedly Reward
            f"3/ What Sponsors Repeatedly Reward:\n\n"
            f"Looking at previous winners from {lead_sponsor}, judges don't care about surface-level API calls.\n"
            ...
            # Tweet 4: The Unfair Advantage Build
        ...
        # ------------------------------------------------------------------- #
        # Format 2: TikTok / Instagram Reels Script (45-second tempo)         #
        # ------------------------------------------------------------------- #
        reels_script = (
            f"[HOOK - 0:00-0:05]: Stop entering hackathons blindly. Here is the exact playbook to win the ${total_prize:,.0f} {name}.\n"
        ...
        # ------------------------------------------------------------------- #
        # Format 4: LinkedIn Thought-Leadership Post                          #
        # ------------------------------------------------------------------- #
        linkedin_post = (
            ...
            f"A deep dive into the ${total_prize:,.0f} {name} across {ecosystem} reveals a stark pattern:\n\n"
        ...
        # ------------------------------------------------------------------- #
        # Step 3: Package into final dictionary and save to database          #
        # ------------------------------------------------------------------- #
        package = {
            "id": f"content_{slug}",
            "opportunity_id": opportunity.get("id"),
            "title": f"How to Win the ${total_prize:,.0f} {name} (Playbook)",
    ...
    def get_all_content_opportunities(self) -> List[Dict[str, Any]]:
        """Returns all drafted content packages."""
